...
def main(args):
    ...
    # attach step-wise event logger (for K2 if present)
    try:
        if getattr(sampler, 'k2_scheduler', None) is not None:
            sampler.k2_scheduler.logger = JSONLLogger(str(root / 'events.jsonl'))
    except Exception:
        pass
    ...
    # write timing.json (wall-clock and config steps)
    try:
        total_sec = time.perf_counter() - t0
        # Prefer actual loaded images if available
        num_images = int(images.shape[0]) if isinstance(images, torch.Tensor) else len(images)
        denom = max(1, num_images * int(args.num_runs))
        # prefer actual used steps (early-stop) if sampler recorded it
        try:
            used_steps = int(getattr(sampler, 'used_anneal_steps', None))
        except Exception:
            used_steps = None
        timing = {
            'total_wall_sec': float(total_sec),
            'per_image_sec': float(total_sec / denom),
            'ode_steps': int(args.sampler.diffusion_scheduler_config.num_steps),
            'anneal_steps_cfg': int(args.sampler.annealing_scheduler_config.num_steps),
            'used_anneal_steps': used_steps if used_steps is not None else (
                int(getattr(getattr(sampler, 'annealing_scheduler', None), 'num_steps', 0)) - 1 if getattr(getattr(sampler, 'annealing_scheduler', None), 'num_steps', None) is not None else None
            ),
        }
        json.dump(timing, open(str(root / 'timing.json'), 'w'), indent=2)
